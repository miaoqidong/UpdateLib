...
/**
 * 简化版检查更新逻辑——不关心 APK 是否存在，发现新版本就提示用户去网站下载。
 */
public class UpdateRepository {
    ...
    public static class CheckResult {
        public enum Type { SKIPPED, FAILED, RATE_LIMITED, NEW_VERSION, UP_TO_DATE }
        public Type type;
    ...
    private static void processRelease(Context context, String currentVersion, GithubRelease release, CheckCallback callback) {
        String compareVersion = UpdateChecker.compareByTag ? release.tagName : release.tagName;
    ...
    private static void processFallback(Context context, String currentVersion, FallbackRelease fbRelease, CheckCallback callback) {
        ...
        if (fbRelease.desContent != null && !fbRelease.desContent.isEmpty()) {
            FallbackChecker.fetchDescription(fbRelease.desContent, notes -> {
                saveAndReturnFallback(context, fbRelease, notes, isNewer, callback);
            });
        } else {
            saveAndReturnFallback(context, fbRelease, "", isNewer, callback);
        }
    }

    private static void saveAndReturnFallback(Context context, FallbackRelease fbRelease,
